...
def preprocess(text):
    ...
    processed_data = []
    # Use a safe lookup for IDs after filtering
...
def score_sentences(sentences_data, unfiltered_map, similarity_threshold=0.3):
    ...
    for i, s_data in enumerate(sentences_data):
        ...
        if original_id == 0:
            structural_bonus = 0.15 # Introduction bonus
        elif original_id == n_original - 1:
            structural_bonus = 0.10 # Conclusion bonus
    ...
    weights = {
        ...
        'global_cohesion_score': 0.15,
        'info_density_score': 0.15, # Give info density a strong weight
        'structural_bonus': 0.15      # Give structure a strong weight
    }
...
def generate_summary_with_mmr(sentences_data, num_clusters, 
                              ...
                              dynamic_compression_std_dev=0.7):
    """
    MODIFIED: Uses importance-weighted equitable allocation for better topic balance.
    """
    ...
    ## NEW ## - Priority #1: Importance-Weighted Equitable Allocation
    ...
    # Second pass: importance-weighted remainder
    if available_slots > 0:
        # Calculate cluster importance based on its best sentence
        cluster_importance = {}
        for cid, sents in clusters.items():
            if sents:
                cluster_importance[cid] = max(s['relevance_score'] for s in sents)
    ...
    for cid in sorted(num_to_pick_from_cluster.keys()): # Sort for deterministic output
...
if __name__ == "__main__":
    ...
    DYNAMIC_COMPRESSION_STD_DEV = 0.6 # Tuned to be slightly more selective
    
    # --- Input Text (The "Time Travel" example) ---
    ...
    print(f"         {num_sents_summary} sentences selected from {len(unfiltered_map)} original sentences")
